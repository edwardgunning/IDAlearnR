...
### Ordered factors
Sometimes the levels of a factor variable have a natural ordering that we want R to recognise. We call these ordinal data, and record them as an ordered factor.
...
We can create an __ordered factor in two ways:
...
When printing the levels, R shows that they have a definite ordering using the `<` symbol. Other than this, you will not notice any other differences from an unordered factor at first, but it can become important later at the modelling stage of a data analysis.
...
```{r var-type-1-quiz}
quiz(
  question('Q1 What is the difference between a factor and an ordinal variable?',
