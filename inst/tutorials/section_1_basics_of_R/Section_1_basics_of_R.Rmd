...
## Indexing
### 
We have already touched on accessing values in a vector by specifying their position with `[]`, called _numerical indexing_. Remember, `v[1]` will select the first value from the vector `v`. We have also seen how we can access elements of a vector using their names.

It is also possible to use __negative indexing__. When a negative number is used to index a vector, all values __except__ for the value in that position are returned.

In this section, we will look more __numerical indexing__, and also introduce __logical indexing__, a powerful technique which uses logical statements which we learned about in the previous section to index vectors.

```{r index-0, exercise=TRUE, exercise.lines = 15}
# Create a vector
v <- c(1, 5, -11, 33)
