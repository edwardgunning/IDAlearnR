...
## Vectors
###
__Atomic vectors__ are one-dimensional arrays which are fundamental for programming in R. For now, we will refer to them simply as __vectors__, noting that we will encounter a more flexible type of vector, called __lists__, later in the course.
...
### Vector types
R defines different vector __types__ based on the values they contain. The most important types of vector are _logical_, _integer_, _double_, and _character_. 
...
- If the values are numbers, R recognises a vector as __numeric__. E.g., `c(1.1, -2, 3)`.
...
We can use the `typeof()` and `class()` functions to find the type of a vector.
...
You can use `c()` to combine multiple vectors into a single vector, but this comes with a health warning. If you are not careful, it may result in __type conversions__. Type conversion (also refered to as __typecasting__) refers to changing an entity of one datatype into another.
...
quiz(caption = "",
  ...
  question('Q2: When you added `c(1, 2)` to `op_v2` what happened?',
    ...
    allow_retry = TRUE,
    correct = "Correct! This is something we need to be _very_ careful of. <br> Even though the vectors are of different length, R will not give us a warning or error message. It will recycle the shorter vector, replicating it up to the same length as the longer vector and then perform element-wise operations. <br> If you are skeptical, check that the following give the same result: <br> `c(1, 2) + op_v2` <br> `c(1, 2, 1, 2) + op_v2` <br> `rep(c(1, 2), times = 2) + op_v2` "
    )
)
```
...
**Be very careful** with the order of operations! 
